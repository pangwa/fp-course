...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f s = State $ \a -> let (a1, s1)= runState s a
                            in (f a1, s1)
...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s b
  (<*>) sf st =
    let newS = \s -> let (f, s1) = runState sf s
                         (a, s2) = runState st s1
                         in (f a, s2)
    in State newS
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s b
  (=<<) f st =
    let newS = \s -> let (a, s1) = runState st s
...
findM pred (x:.xs) =
  pred x >>= \r -> if r then pure (Full x) else findM pred xs
